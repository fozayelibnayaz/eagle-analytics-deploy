...
def render_cs_analytics():
        ...
        try:
            ...
            if ch["by_month"]:
                ...
                st.plotly_chart(px.bar(df, x="Month", y="Churned", color="Churned",
                              ...
                              width='stretch')
            if ch["customers"]:
                st.dataframe(pd.DataFrame(ch["customers"]), width='stretch', hide_index=True)
    ...
    with sections[1]:
        try:
            ...
            with c1:
                ...
                st.plotly_chart(px.pie(df, values="Count", names="Plan", title="Plans"), width='stretch')
                st.dataframe(df, width='stretch', hide_index=True)
            with c2:
                df = pd.DataFrame([{"Status":k,"Count":v} for k,v in h["statuses"].items()])
                st.plotly_chart(px.bar(df, x="Status", y="Count", color="Count", title="Status"), width='stretch')
            c3,c4 = st.columns(2)
            with c3:
                ...
                st.plotly_chart(px.bar(df, x="Size", y="Count", color="Count", color_continuous_scale="Blues",
                                       title="Company Size"), width='stretch')
            with c4:
                df = pd.DataFrame([{"Fit":k,"Count":v} for k,v in h["perfect_fit"].items()])
                st.plotly_chart(px.pie(df, values="Count", names="Fit", title="Perfect Fit Customer"), width='stretch')
    ...
    with sections[2]:
        try:
            ...
            st.dataframe(pd.DataFrame(s["top_50_streamers"]), width='stretch', hide_index=True)
        ...
        try:
            ...
            with c1:
                ...
                st.plotly_chart(px.pie(df, values="Count", names="Type", title="Connection Success",
                                       color_discrete_map={"Success":"green","Failure":"red"}), width='stretch')
            with c2:
                df = pd.DataFrame({"Type":["Success","Failure"],
            ...
            st.dataframe(pd.DataFrame(sn["top_30_clients"]), width='stretch', hide_index=True)
            st.subheader("🚨 High Failure Clients (need attention)")
            if sn["high_failure_clients"]:
                st.dataframe(pd.DataFrame(sn["high_failure_clients"]), width='stretch', hide_index=True)
    ...
    with sections[4]:
        try:
            ...
            with c1:
                ...
                if not df.empty:
                    st.plotly_chart(px.pie(df, values="Count", names="Color", title="Color"), width='stretch')
            with c2:
            ...
            if s1["dormant_paying"]:
                st.dataframe(pd.DataFrame(s1["dormant_paying"]), width='stretch', hide_index=True)
    ...
    with sections[5]:
        try:
            ...
            if sub["ending_in_30d"]:
                st.dataframe(pd.DataFrame(sub["ending_in_30d"]), width='stretch', hide_index=True)
            st.subheader("💎 Long-term Loyal (12+ payments)")
            if sub["long_term_customers"]:
                st.dataframe(pd.DataFrame(sub["long_term_customers"]), width='stretch', hide_index=True)
    ...
    with sections[6]:
        try:
            ...
            if st_trend:
                ...
                st.plotly_chart(px.line(df, x="period", y="stream_time", markers=True), width='stretch')
    ...
    with sections[8]:
        try:
            ...
            c1,c2,c3 = st.columns(3)
            with c1:
                st.dataframe(pd.DataFrame([{"Plan":k,"Count":v} for k,v in n["by_plan"].items()]), width='stretch', hide_index=True)
            with c2:
                st.dataframe(pd.DataFrame([{"Size":k,"Count":v} for k,v in n["by_size"].items()]), width='stretch', hide_index=True)
            with c3:
                st.dataframe(pd.DataFrame([{"Fit":k,"Count":v} for k,v in n["by_fit"].items()]), width='stretch', hide_index=True)
    ...
    with sections[10]:
        try:
            ...
            if ph["call_statuses"]:
                ...
                st.plotly_chart(px.bar(df, x="Status", y="Count", color="Count"), width='stretch')
    ...
    with sections[13]:
        try:
            ...
            if ra["growing"]:
                st.dataframe(pd.DataFrame(ra["growing"]), width='stretch', hide_index=True)
            st.subheader("📉 Declining - URGENT outreach")
            if ra["declining"]:
                st.dataframe(pd.DataFrame(ra["declining"]), width='stretch', hide_index=True)
    ...
    with sections[14]:
        try:
            ...
            if vt["vip_customers"]:
                st.dataframe(pd.DataFrame(vt["vip_customers"]), width='stretch', hide_index=True)
