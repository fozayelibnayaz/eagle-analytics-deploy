...
def render_ga4_pattern_analysis():
    ...
    if geo_data:
        ...
        with col_b:
            ...
            st.dataframe(df_geo_display, width='stretch', hide_index=True)
    ...
    if daily_data:
        ...
        with col_s:
            ...
            st.dataframe(top10[["date","sessions","users"]], width='stretch', hide_index=True)
        with col_d:
            st.markdown("**📉 Bottom 10 Slow Days**")
            bottom10 = df.sort_values("sessions").head(10)
            st.dataframe(bottom10[["date","sessions","users"]], width='stretch', hide_index=True)

        if len(spikes) > 0:
            with st.expander(f"🚀 All {len(spikes)} spike days (>2 sigma above mean)"):
                st.dataframe(spikes[["date","sessions","users"]], width='stretch', hide_index=True)
